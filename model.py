...
df_users_scaled[activity_columns] = scaler.fit_transform(df_users[activity_columns])  # Ensure scaling happens here

# Define recommendation function
def recommend_users(user_id, top_n=5, location_filter=None):
    # Ensure the user_id is valid
    if user_id not in df_users_scaled["User_ID"].values:  # Ensure the column is named correctly
    ...
    user_data = df_users_filtered[df_users_filtered["User_ID"] == user_id][activity_columns]
    other_users = df_users_filtered[df_users_filtered["User_ID"] != user_id][activity_columns]
    ...
    recommended_users = df_users_filtered.iloc[similar_users_indices]["User_ID"].tolist()
